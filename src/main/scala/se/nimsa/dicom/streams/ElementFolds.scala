    ...
    DicomFlowFactory.create(new DeferToPartFlow[Element] with TagPathTracking[Element] {
      ...
      override def onPart(part: DicomPart): List[Element] = part match {
        case header: DicomHeader =>
          currentValue = tagPath.map(tp => Element(tp, header.bigEndian, header.vr, header.explicitVR, header.length, ByteString.empty))
          Nil
        case fragments: DicomFragments =>
          currentFragment = tagPath.map(tp => Element(tp, fragments.bigEndian, fragments.vr, explicitVR = true, fragments.length, ByteString.empty))
          Nil
